...
export const paymentVerfication = async (req, res) => {
    try {
      ...
      const userData = encodeURIComponent(
        ...
        res.redirect(
          `https://darling-kelpie-39d5c8.netlify.app//subscription/success?reference=${recieptData}&user=${userData}`
        );
      } else {
        return res.redirect(
          `https://darling-kelpie-39d5c8.netlify.app//subscription/cancel?reference=${razorpay_payment_id}`
        );
      }
    } catch (err) {
      console.log(err);
